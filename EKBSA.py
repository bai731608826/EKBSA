...
class BertClassifier(nn.Module):
    ...
    def forward(self, src, label, mask, pos=None, vm=None, dep=None):
        """
        Args:
        ...
        if not self.use_vm:
            vm = None
        output = self.encoder(emb, mask, vm, dep)
        # Target.
    ...
    for line_id, line in enumerate(sentences):
        ...
        if len(line) == 2 :
            ...
            if label < 2:
                ...
                label = 1
            text = line[0]
            tokens, pos, vm, types, dep = kg.add_knowledge_with_vm([text], add_pad=True, max_length=args.seq_length)
            tokens = tokens[0]
            pos = pos[0]
            mask = types[0]
            vm = vm[0].astype("bool")
            dep = dep[0].astype("bool")

            token_ids = [vocab.get(t) for t in tokens]
            #mask = [vocab.get(t) if t != PAD_TOKEN else 0 for t in types]

            dataset.append((token_ids, label, mask, pos, vm, dep))
        else:
            continue
    return dataset


def main(epoch, kg_name, dist):
    parser = argparse.ArgumentParser(formatter_class=argparse.ArgumentDefaultsHelpFormatter)

    # Path options.
    parser.add_argument("--pretrained_model_path", default="./models/google_model.bin", type=str,
                        help="Path of the pretrained model.")
    parser.add_argument("--output_model_path", default="./models/embedding_model.bin", type=str,
                        help="Path of the output model.")
    parser.add_argument("--vocab_path", default="./models/google_vocab.txt", type=str,
                        help="Path of the vocabulary file.")
    parser.add_argument("--train_path", default="./datasets/duihua/train.csv", type=str,
                        help="Path of the trainset.")
    parser.add_argument("--dev_path", default="./datasets/duihua/dev.csv", type=str,
                        help="Path of the devset.")
    parser.add_argument("--test_path", default="./datasets/duihua/test.csv", type=str,
    ...
    set_seed(args.seed)
    args.kg_name = kg_name
    args.epochs_num = epoch
    ...
    # Datset loader.
    def batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms, deps):
        ...
        for i in range(instances_num // batch_size):
            ...
            deps_batch = deps[i * batch_size: (i + 1) * batch_size]
            yield input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, deps_batch
        if instances_num > instances_num // batch_size * batch_size:
            ...
            deps_batch = deps[instances_num // batch_size * batch_size:]
    ...
    kg = KnowledgeGraph(spo_files=spo_files, dist=dist, predicate=True)
    ...
    def read_dataset(path, workers_num=1):
        ...
        with open(path, mode='r', encoding="utf-8") as f:
            reader = csv.reader(f)
            for line_id, line in enumerate(reader):
                sentences.append([line[3],line[4][-1]])
    ...
    def evaluate(args, is_test, metrics='Acc'):
        ...
        dep = [example[5] for example in dataset]
        ...
        if not args.mean_reciprocal_rank:
            for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, dep_batch) in enumerate(
                    batch_loader(batch_size, input_ids, label_ids, mask_ids, pos_ids, vms, dep)):

                # vms_batch = vms_batch.long()
                vms_batch = torch.LongTensor(vms_batch)
                dep_batch = torch.LongTensor(dep_batch)
                ...
                with torch.no_grad():
                    try:
                        loss, logits = model(input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, dep_batch)
                    except:
    ...
    print("dep")
    dep = [example[5] for example in trainset]
    ...
    for epoch in range(1, args.epochs_num + 1):
        ...
        for i, (input_ids_batch, label_ids_batch, mask_ids_batch, pos_ids_batch, vms_batch, dep_batch) in enumerate(
            ...
            vms_batch = vms_batch.to(device)
            dep_batch = dep_batch.to(device)
            loss, _ = model(input_ids_batch, label_ids_batch, mask_ids_batch, pos=pos_ids_batch, vm=vms_batch, dep = dep_batch)
